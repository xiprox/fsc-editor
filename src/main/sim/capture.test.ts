...
afterEach(async () => {
  // Awaited, and then retried anyway: Windows will not delete a file whose
  // handle is still open, and a session that rolled leaves a close pending per
  // part — only the last of which this waits for. Without the retries a test
  // that happened to leave a part mid-flush fails in *its teardown*, on a
  // runner loaded enough that the flush lands after the removal, which says
  // nothing about the test it is reported against.
  await stopCapture()
 ...
 *
 * Awaited rather than slept on. This was a flat 50ms, which is a bet on how
 * loaded the machine is: the close, the rename and the eviction all happen on
 * the OS's schedule, and on a CI runner they land after the assertion. That is
 * the whole of the intermittent failure this file was known for — a different
 * test each run, all of them passing in isolation.
 *
 * It settles the part that was live. A burst of rolls queues a close per part
 * and those land in their own time, so what happens to the parts *behind* this
 * one is still polled for, below.
 */
...
describe("bounds", () => {
  ...
  /**
   * Polled for the same reason as the test above, and then asserted: the prune
   * that decides this one is the one the *last* part's close callback fires,
   * which is the callback that lands after the session has already ended.
   */
...
/**
 * Asserted against files put there by hand rather than by a session, because
 * what is being checked is an order, and a session long enough to have one
 * takes a second to write and evicts most of it on the way.
 */
describe("captureFiles", () => {
  const at = "2026-08-23T09-51-02"
  const later = "2026-08-23T11-04-30"

  function put(...names: string[]): void {
    mkdirSync(join(dir, CAPTURE_DIR), { recursive: true })
    for (const name of names) writeFileSync(join(dir, CAPTURE_DIR, name), "")
  }

  /**
   * `.p10` sorts before `.p9` as text. Sorted by name, a session past nine
   * parts is in neither the disk's order nor its own — which is the order
   * `prune` evicts in and the order a debug report is assembled in.
   */
  it("orders a session's parts by number, not by name", () => {
    put(`${at}.p11.ndjson`, `${at}.p9.ndjson`, `${at}.ndjson`, `${at}.p10.ndjson`)

    expect(captureFiles(dir).map((file) => basename(file))).toEqual([
      `${at}.ndjson`,
      `${at}.p9.ndjson`,
      `${at}.p10.ndjson`,
      `${at}.p11.ndjson`,
  ...
  /** And the session itself still comes first, aircraft in the name or not. */
  it("orders sessions by when they started", () => {
    put(`${later}.ndjson`, `${at}-pa24-250.p2.ndjson`, `${at}-pa24-250.ndjson`)

    expect(captureFiles(dir).map((file) => basename(file))).toEqual([
      `${at}-pa24-250.ndjson`,
      `${at}-pa24-250.p2.ndjson`,
      `${later}.ndjson`,
    ])
  })
})
